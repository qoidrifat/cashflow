...
- **Alert rules**: 7 seed default (ai_cost_daily, gmail_sync_failures, agent_search_error_rate, ocr_failure_rate, cache_hit_rate — deteksi degradasi LRU cache: hit rate < 50% dalam 60 menit, fraud_flags — lonjakan flag mencurigakan, ai_cost_monthly — biaya estimasi AI 30 hari > Rp 100k), evaluasi windowed di `checkAlerts()`, idempotent seed.
...
- **Gmail review channel (user-facing, di luar dashboard)**: `server/services/gmailNotifier.js` — hasil aksi review Gmail (approve/reject/duplicate/failed) dikirim ke channel eksternal agar user tahu walau app tidak terbuka: (a) **webhook** POST JSON ke `GMAIL_WEBHOOK_URL` (fallback `ALERT_WEBHOOK_URL`) + (b) **email** via SMTP (`SMTP_HOST/PORT/USER/PASS/FROM`, nodemailer — vendor-agnostic) ke email user pelaku aksi. Dipicu server-side dari `POST /api/notifications` (gate ketat: hanya `metadata.source === 'gmail_review'` + `metadata.emailId` — ringkasan harian `gmail-review-<tanggal>` TIDAK memicu). In-flight guard per emailId, never-throw. Env baru di `.env.example`.
